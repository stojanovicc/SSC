...
export default function ZakazaniStudenti({ treningId, treningSports }) {
    ...
    const getApplicants = async () => {
        const response = await fetch("http://localhost:7240/Trener/GetAppliedStudentsTrening/" + treningId, {
            credentials: "include"
        });
        if (response.ok) {
            ...
            if (data.succeeded) {
                console.log(data)
                setApplicants(data.trening.applicants);
            }
        }
    }
    ...
    return (
        <Grid container sx={{ width: 1 }}>
            {
                applicants.length == 0 ?
                    <Typography>Nema zakazanih </Typography> :
